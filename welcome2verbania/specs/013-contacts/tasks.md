...
- [x] T013-01 In `index.html` `<script>` block, add `CONTENT.sections.contacts` object:
  ```js
  ...
  ```
- [x] T013-02 Add translatable string keys to `CONTENT.strings.en` (+ `it`/`de` placeholders):
...
- [x] T013-03 Add `<div class="screen" id="contacts" hidden>` to `index.html`; prepend `createSectionHeader('contacts_title')`
- [x] T013-04 Render the contact card from JS (`renderContacts()`):
...
- [x] T013-05 Tap "Call" button on real device → phone dialler opens with +39 335 6328554 pre-filled ✅
- [x] T013-06 Tap "WhatsApp" button → WhatsApp opens with Marco Melli's number pre-filled ✅
- [x] T013-07 Section renders correctly at 360 dp; back button returns to home ✅
- [x] T013-08 Works fully offline (WhatsApp deep link uses OS, no network needed for rendering) ✅
...
- [x] T013-NEW Translate Signal note to Italian and German (added to `it` and `de` string keys)
