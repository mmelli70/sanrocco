...
- [x] T004-01 Add `CONTENT.sections.restaurants` array (11 entries) to `index.html`:
  ```js
  ...
  ```
- [x] T004-02 Add string keys to `CONTENT.strings.en`: `restaurants_title`, `restaurants_openMaps`, `restaurants_favourite`
...
- [x] T004-05 [P] Write `renderFavouriteBadge(label)` JS helper: returns `<span class="badge badge-favourite">★ {label}</span>` or empty string if not favourite
...
- [x] T004-06 Add `<div class="screen" id="restaurants" hidden>` to `index.html`; prepend `createSectionHeader('restaurants_title')`
- [x] T004-07 Write `renderRestaurants()`: loop over `CONTENT.sections.restaurants[]`, for each entry build a `.card`:
...
- [x] T004-08 Call `renderRestaurants()` on `DOMContentLoaded`
...
- [x] T004-09 All 12 cards render at 360 dp without overflow ✅
- [x] T004-10 Donna Francesca, Cantina Leonardo, La Pescheria, La Terrazza show ★ favourite badge ✅
- [x] T004-12 Maps links open on device ✅
- [x] T004-13 Back button returns to home ✅
...
- [x] T004-NEW Add Italian and German description fields (`it`, `de`) to each restaurant entry
- [x] T004-NEW2 Add La Terrazza as 12th restaurant (added during build)
